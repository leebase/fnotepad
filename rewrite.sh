...
: do-copy ( -- )
  sel-active? if
    ...
    0 ?do
      sel-lower i + \ logical address
      \ Map logical address to physical
...
: do-cut ( -- )
  sel-active? if
    do-copy  \ copies text, clears sel, clipboard-len is set
    \ delete clipboard-len chars starting at sel-lower logical pos
    \ first move cursor to sel-lower+clipboard-len (upper bound)
...
: draw-char ( c -- )
  dup 10 = if drop 13 emit 10 emit else emit then ;

\ Draw the text body.  Emit highlight ANSI only on transitions.
: draw-text ( -- )
  3 1 move-cursor
  0 \ current highlight state (0=normal 1=highlighted)

  \ --- Before-gap block ---
  gap-start @ text-buffer > if
    gap-start @ text-buffer ?do
      i text-buffer - in-sel? \ is logical index in selection?
      over <> if               \ if this differs from current state
        dup if                 \ we're leaving highlight
          ansi-start ." 7m"
        else
          ansi-start ." 0m"
        then
        dup  \ copy new state (already on stack as the "in-sel?" result)
      then
      drop  \ drop old-state copy
      i text-buffer - in-sel? swap drop  \ get current in-sel? again for state tracking
      i c@ draw-char
    loop
  then
  ...
  gap-end @ text-buffer MAX-BUFFER + < if
    text-buffer MAX-BUFFER + gap-end @ ?do
      \ logical address = cursor-pos + (i - gap-end)
      cursor-pos i gap-end @ - + in-sel?
      over <> if
        dup if ansi-start ." 7m" else ansi-start ." 0m" then
        dup
      then
      drop
      cursor-pos i gap-end @ - + in-sel? swap drop
      i c@ draw-char
    loop
  then
  drop
